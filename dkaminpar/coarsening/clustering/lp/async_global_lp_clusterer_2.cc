/*******************************************************************************
 * @file:   async_global_lp_clusterer_2.cc
...
class AsyncGlobalLPClusteringImpl2 final
    : public InOrderLabelPropagation<AsyncGlobalLPClusteringImpl2, AsyncGlobalLPClusteringConfig2>,
      public NonatomicOwnedClusterVector<NodeID, GlobalNodeID> {
  SET_DEBUG(false);

  using Base = InOrderLabelPropagation<AsyncGlobalLPClusteringImpl2, AsyncGlobalLPClusteringConfig2>;
...
public:
  ...
  void move_node(const NodeID lu, const ClusterID gcluster) {
    ...
    node_move << "Node Move(rank, node_id, g_node_id, is_interface_node, old_c, new_c): (" << rank << ", " << lu << ", " << _graph->local_to_global_node(lu) << ", " << is_interface_node(lu) << ", " << _changed_label[lu] << ", " << gcluster << ")" << std::endl;
    ...
    mpi::graph::sparse_alltoall_interface_to_pe_clustering<Message>(
        *_graph,
        ...
        [&](auto &&buffer, const PEID owner) {
          int rank = mpi::get_comm_rank(_graph->communicator());
          if (rank != owner && buffer.size() != 0) {
            _total_received_labels += buffer.size();
            std::cout << "rank = " << rank << ", owner = " << owner << ", received buffersize = " << buffer.size() << std::endl;
          }
  ...
  */
  ...
  GlobalNodeID handle_labels(const int from, const int to, const NodeID local_num_moved_nodes, std::vector<NoinitVector<Message>> &msgBuffers, const int size) {
    ...
    for (int i = 0; i < size; ++i) {
      ...
      std::cout << "MsgBuffer contains messages: rank = " << rank << ", size = " << buffer.size() << std::endl;
      int rank = mpi::get_comm_rank(_graph->communicator());
      ...
      for (size_t j = 0; j < buffer.size(); ++j) {
        ...
        std::cout << "rank = " << rank << ", " << "element in buffer: " << owner_lnode << ", " << new_gcluster << std::endl;
        ...
        std::cout << "move ghost node to new cluster: rank = " << rank << ", " << lnode << "(" << _graph->local_to_global_node(lnode) << ")" << " to " << new_gcluster << std::endl;
