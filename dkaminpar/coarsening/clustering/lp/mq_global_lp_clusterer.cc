...
struct MQGlobalLPClusteringConfig : public LabelPropagationConfig {
...
class MQGlobalLPClusteringImpl final
    : public InOrderLabelPropagation<MQGlobalLPClusteringImpl, MQGlobalLPClusteringConfig>,
      public NonatomicOwnedClusterVector<NodeID, GlobalNodeID> {
  SET_DEBUG(false);

  using Base = InOrderLabelPropagation<MQGlobalLPClusteringImpl, MQGlobalLPClusteringConfig>;
...
public:
  explicit MQGlobalLPClusteringImpl(const Context &ctx)
...
private:
  ...
  */
  ...
  void synchronize_ghost_node_clusters(const NodeID from, const NodeID to) {
    ...
    // TODO exchange this for mq comm

    std::vector<LabelMessage> buffer;
    for (NodeID lnode = from; lnode < to; ++lnode) {
        if (_changed_label[lnode] != kInvalidGlobalNodeID) {
            buffer.push_back({ .owner_lnode = lnode, .new_gcluster = cluster(lnode)});
        }
    }
    ...
    // allocate buffers
    std::vector<NoinitVector<LabelMessage>> send_buffers(pes);
    tbb::parallel_for<PEID>(0, pes, [&](const PEID pe) {
      send_buffers[pe].resize(num_messages[pe]);
    });
    ...
    for (NodeID u = from; u < to; ++u) {
      ...
      for (const auto [e, v] : (*_graph).neighbors(u)) {
        ...
        if (created_for_pe[pe] == 1) {
          continue;
        }
        created_for_pe[pe] = 1;

        const auto slot = --num_messages[pe];

        send_buffers[pe][slot] = { u, cluster(u) };
      }
    }

    tbb::parallel_for<PEID>(0, pes, [&](const PEID pe) {
      _queue.post_message(std::move(send_buffers[pe]), pe);
    });
...
MQGlobalLPClusterer::MQGlobalLPClusterer(const Context &ctx)
    : _impl{std::make_unique<MQGlobalLPClusteringImpl>(ctx)} {}

MQGlobalLPClusterer::~MQGlobalLPClusterer() = default;

void MQGlobalLPClusterer::initialize(const DistributedGraph &graph) {
  _impl->initialize(graph);
}

MQGlobalLPClusterer::ClusterArray &MQGlobalLPClusterer::cluster(
